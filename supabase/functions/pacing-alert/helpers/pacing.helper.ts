  ...
  static buildProjectRows(projects: ProjectAnalysis[]): string {
    return projects
      .map((p) => {
        ...
        return `
          <tr>
            <td style="padding: 12px 0; border-bottom: 1px solid ${EMAIL_THEME.DIVIDER_COLOR};">
              <strong style="color: ${EMAIL_THEME.PRIMARY_TEXT};">${p.name}</strong><br>
              <span style="color: ${statusColor}; font-size: 12px; font-weight: 700;">${statusLabel}</span>
            </td>
            <td style="padding: 12px 0; border-bottom: 1px solid ${EMAIL_THEME.DIVIDER_COLOR}; text-align: right;">
